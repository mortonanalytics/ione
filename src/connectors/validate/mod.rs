...
pub async fn dispatch(kind: &str, name: &str, config: &Value) -> ValidateResult {
    match kind {
        "rust_native" => match name.to_ascii_lowercase().as_str() {
            "nws" => nws::validate(config).await,
            "firms" => firms::validate(config).await,
            "s3" | "fs_s3" | "s3_fs" => s3::validate(config).await,
            "slack" => slack::validate(config).await,
            "irwin" => irwin::validate(config).await,
            other => Err(ValidateErr::new(
                "unknown_rust_native_provider",
                &format!(
                    "No rust_native provider named '{other}'. Known: nws, firms, s3, slack, irwin."
                ),
            )
