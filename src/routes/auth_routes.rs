...
use crate::{
    auth::{
        clear_session_set_cookie, mode_from_env, pkce_challenge, random_url_safe_string, AuthMode,
...
pub async fn callback(
    ...
    // In production this would exchange the code via openidconnect crate.
    // Without a live IDP we return 400 with a clear message.
    Err(AppError::BadRequest(
        "code exchange not available without a live IDP".into(),
    ))
}
