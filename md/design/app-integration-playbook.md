...
- `document` — `metadata.download_url` (an **https** URL the IONe shell embeds/links directly; IONe does **not** proxy it — same contract as map `tile_url`) + `metadata.mime_type`; optional `metadata.file_size_bytes`, `metadata.last_modified`. The `download_url` **MUST be `https` and retrievable by the operator's browser without IONe's delegated peer token** — i.e. public, presigned/time-limited, or authenticated by a cookie on the peer's own origin — and **must stay valid ≥ 5 min** after `resources/list` (the operator clicks later). IONe rejects non-https and link-local URLs and does not inject auth (no proxy). **No `resources/read` body is defined** for documents — `resources/list` metadata is sufficient (metadata-only path, like `map`). IONe inline-embeds `application/pdf` in a sandboxed iframe; other MIME types render an "open in new tab" link. (Promoted to v0.1 scope per [md/design/document-view.md](document-view.md).)
