...
| 3 | Signed webhook sender (§3) | The kit runs a receiver implementing IONe's verification and checks: `X-IONe-Signature` grammar (only `t`/`v1`, no repeats), `v1` is exactly 64 lowercase hex, the HMAC over `t_ascii ++ "." ++ raw_body`, `abs(now − t) ≤ 300`, `abs(t − occurred_at) ≤ 30`, the path `peer_id`, the 256 KiB body cap, and every §3.3 envelope constraint. `approval_required` is checked as **optional** per §3.3 — absent passes and is reported as defaulting to `false`; present-but-not-boolean fails. |
...
history is kept because it explains why the regressions that pin them exist.
Verify against the cited source before citing a finding as current.
