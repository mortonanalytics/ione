...
- **Authz tiers (pre-RBAC):** list endpoint = workspace member (matches today's exposure); aggregates + export = admin (coc ≥ 80), because bulk retrieval of every member's actions is a materially different exposure than a 200-row recent list. Revisit when RBAC lands (the planned `audit:read` permission — see `md/design/rbac-scaffolding.md`).
