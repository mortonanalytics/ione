...
2. **`app.current_org_id` RLS activation (HP-M4, partially closed 2026-07-25).** Migration 0050 adds `FORCE ROW LEVEL SECURITY` to all eleven org-scoped tables and introduces a restricted, non-SUPERUSER/non-`BYPASSRLS` role (`ione_app`); `src/rls.rs::org_scoped_tx` sets `app.current_org_id` per transaction. Isolation is proven under that role by `tests/rls_enforcement_integration.rs`. **`service_account_tokens` is NOT among the migrated repositories**, so for this design's token table the application-layer `WHERE org_id = $1` is still the only enforcement, and it is still bypassed entirely under the default `ione` connection. Named so it is not mistaken for active defense. See md/design/identity-broker.md for the precise coverage boundary.
