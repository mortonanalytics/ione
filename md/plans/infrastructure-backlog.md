...
- **Federated catalog search (lexical-first).** **Partial — pending founder walkthrough** (code-complete on `feature/federated-catalog-search`: `peer_catalog_entries` index maintained off `refresh_manifest_if_changed` with content-hash delta + orphan delete; RBAC-pre-filtered weighted-FTS + trigram search shared by `GET /workspaces/:id/catalog-search` (always-visible Catalog panel) and the `search_catalog` MCP tool; pre-filter mirrors `route_tool_call` so visibility == invocation capability; reserved `vector(768)` for hybrid v2. Design `md/design/federated-catalog-search.md`, plan `md/plans/federated-catalog-search-plan.md`, requirements `md/requirements/active/federated-catalog-search.md`). Backs the §2.4 "surfaced upon relevance" claim. Vector/semantic reranking deferred to hybrid v2 behind a measured lexical miss (Open Question 1).
...
- **Auto-exec policy DSL.** **Partial — pending walkthrough** (`feature/auto-exec-governance`): `auto_exec_policies` table + RBAC-scoped CRUD + Policies panel (`md/design/auto-exec-governance.md`). Two trigger predicates (title prefix, severity ceiling) by design — no rules language.
- **Audit the auto-exec bypass guard.** **Partial — pending walkthrough**: floor verified non-bypassable (design Devil's Advocate); fallback arms coerced to Draft + AC-9 regression test (`feature/auto-exec-governance`).
